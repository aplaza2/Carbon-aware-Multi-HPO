...
class ExperimentManager:
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        try:
            tracker = EmissionsTracker(
                    project_name="Optuna-RL",
                    experiment_id=f"{self.env_name}_{self.algo}_trial_{trial.number}",
                    tracking_mode="process",
                    log_level="error",
                    output_file=f"{self.env_name}_{self.mode}_emissions.csv",
                    output_dir=self.optuna_results_path
            )
            tracker.start()
        ...
        json_file_name = f"{self.env_name}_{self.mode}_results.json"
        output_path = os.path.join(self.optuna_results_path, json_file_name)
    ...
    def hyperparameters_optimization(self) -> None:
        ...
        study = optuna.create_study(
            ...
            load_if_exists=True,
            directions=["maximize", "minimize"] if self.mode == "multi" else None,
            direction="maximize" if self.mode == "single" else None
        )
